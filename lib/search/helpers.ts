...
export function parsePriceFilter(qRaw: string, budgetMaxParam: number | null): { min: number | null; max: number | null } {
  ...
  if (max == null && /billig|budget/.test(txt)) max = 500;
...
export function expandQuery(q: string): string[] {
  ...
  const eq: Record<string, string[]> = {
    skateboard: ["skate", "board", "deck"],
    longboard: ["cruiser", "skateboard"],
    løbehjul: ["scooter", "stunt scooter", "trick scooter"],
    loebehjul: ["scooter", "løbehjul"],
    hjelm: ["helmet"],
    helmet: ["hjelm"],
    bmx: ["freestyle"],
    beskyttelse: ["knee pad", "knæbeskytt", "albue"],
  };
  ...
  if (!set.size) set.add("skateboard");
...
export function parseXMLProducts(xml: string, merchant: string): FeedProduct[] {
  ...
  for (const b of blocks) {
    ...
    out.push({
      ...
      category: decodeHTMLEntities(categoryRaw),
      ...
      _search: normalize([title, desc, categoryRaw, brand, merchant].filter(Boolean).join(" ")),
    });
  }
  return out;
}
